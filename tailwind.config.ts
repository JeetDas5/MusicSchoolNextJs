...
// @ts-expect-error: flattenColorPalette is not exported from tailwindcss/lib/util
...
function addVariablesForColors({
  ...
  const allColors = flattenColorPalette(theme("colors")) as {
    [key: string]: string;
  };
...
function addSvgPatterns({
  ...
  matchUtilities: (
    ...
    options: { values: { [key: string]: string }; type: "color" | "length" | "percentage" }
  ) => void;
  theme: (key: string) => { [key: string]: string };
}) {
  matchUtilities(
